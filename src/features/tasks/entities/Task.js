...
const normalizeEventPropsFnMap = {
  TaskCreated: {
    _taskID: Number,
    _timestamp: _timestamp => dayjs.unix(_timestamp).toISOString(),
  },
  TaskAssigned: {
    _taskID: Number,
    _timestamp: _timestamp => dayjs.unix(_timestamp).toISOString(),
...
export const isIncomplete = (
  { status, lastInteraction, submissionTimeout, lifecycleEvents } = {},
  { currentDate = new Date() } = {}
) => {
  ...
  if ([TaskStatus.Created, TaskStatus.Assigned].includes(status)) {
    return dayjs(lastInteraction).add(submissionTimeout, 'second').isBefore(dayjs(currentDate));
  }
 ...
 */
export const registerAssignment = produce((task, { account }) => {
 ...
 * @return {Task} The updated task object
 */
export const registerChallenge = produce((task, { account }) => {
 ...
 * @return {Task} The updated task object
 */
export const registerApproval = produce(task => {
  task.status = TaskStatus.Resolved;
  task.ruling = DisputeRuling.TranslationApproved;
});
 ...
 * @return {Task} The updated task object
 */
export const registerReimbursement = produce(task => {
  task.status = TaskStatus.Resolved;
  task.ruling = DisputeRuling.TranslationRejected;
});
